...
exports.sendOrderRequestEmail = functions.database.ref('/{OrderType}/{id}/history/{index}').onCreate((snapshot, context) => {
    ...
    admin.database().ref(OrderType+"/"+id).once('value', (snap)=>{
        const item = snap.val();
        item.key = id;
        console.log(item)
    ...
    <ul>
        <li class="section">
            ...
            <ul class="section-content">
                ...
                <li><span class="sub-title">Request Time</span><span class="sub-value">${toDate(item.requestTime)}</span></li>
                <li><span class="sub-title">Last Updated</span><span class="sub-value">${toDate(item.lastUpdateTime)}</span></li>
            </ul>
        </li>
            ...
            <ul>
                <li class="section">
                    ...
                    <ul class="section-content">
                        ...
                        <li><span class="sub-title">Current Status</span><span class="sub-value" style="color:crimson">${item.status}</span></li>
                        <li><span class="sub-title">Order Time</span><span class="sub-value">${toDate(item.requestTime)}</span></li>
                        <li><span class="sub-title">Last Updated</span><span class="sub-value">${toDate(item.lastUpdateTime)}</span></li>
                    </ul>
                </li>
...
const toDate=(timestamp)=>moment(timestamp).format("ddd, MMM/DD/YYYY, h:mm A")
const getHistoryArray = (array = []) => {
    let result = ``
    array.map(historyItem => {
        let history = `<li><span class="sub-title">${historyItem.action}</span><span class="sub-value">${toDate(historyItem.time)}</span></li>`;
        if (history.more) {
